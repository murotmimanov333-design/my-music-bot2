...
# Автоматтык түрдө керектүү китепканаларды текшерүү жана орнотуу
try:
...
# --- БАЗА ДАННЫХ (Render Disk үчүн жол) ---
# Эгер Render'да Disk кошсоңуз /data/ ичине сакталат, болбосо локалдык сакталат
DB_DIR = '/data'
if os.path.exists(DB_DIR):
    DB_PATH = os.path.join(DB_DIR, 'rdno_bot.db')
else:
    DB_PATH = 'rdno_bot.db'
...
async def admin_give(message: types.Message):
    try:
        ...
        await message.answer(f"✅ Для ID {t_id} лимит 5 млн открыт.\nДобавлено: {amount} 🌑")
    except: pass
...
async def transfer(message: types.Message):
    ...
    try:
        ...
        if amount > limit: return await message.answer(f"❌ Лимит: {limit} 🌑")
...
async def roulette(message: types.Message):
    ...
    d, c = (f"{num}🔴", "красное") if num in red_numbers else (("0💚", "зеро") if num == 0 else (f"{num}⚫", "черное"))
